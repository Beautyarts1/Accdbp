 ...
 * @author Minami
 */
public final class BankPaymentOpCn {

    DefaultTableModel dtm = new DefaultTableModel();
    public static String id = "";
    String idtable;
    BankPaymentOpView pane;
    ...
    public BankPaymentOpCn(BankPaymentOpView pane) {
        this.pane = pane;
    ...
    private void loaddata() {
        try {
            String query = "SELECT a.BPM_DOC_NO, a.BPM_DATE_TRANS, a.BPM_REF_NO, a.BPM_DATE_REF, "
                 + "a.BPM_ACC,b.ACC_NAME,a.BPM_DATE_CREATED FROM TB_BP_MASTER a "
                 + "INNER JOIN TB_ACC b ON a.BPM_ACC=b.ACC_CODE WHERE a.BPM_DOC_NO=?";
            PreparedStatement pres = c.cn().prepareStatement(query);
            pres.setString(1, id);
            ResultSet res = pres.executeQuery();
            while (res.next()) {
                idtable = res.getString("BPM_DOC_NO");
                pane.eddoc_no.setText(res.getString("BPM_DOC_NO"));
                pane.eddate_trans.setDate(res.getDate("BPM_DATE_TRANS"));
                pane.edref_no.setText(res.getString("BPM_REF_NO"));
                pane.edref_date.setDate(res.getDate("BPM_DATE_REF"));
                pane.edaccount.setText(res.getString("BPM_ACC"));
            ...
            String query2 = "SELECT a.BPD_ID, a.BPD_BPM_MASTER, a.BPD_ACC,b.ACC_NAME, "
                 + "a.BPD_AMOUNT, a.BPD_DESC, a.BPD_DATE_CREATED FROM "
                 + "TB_BP_DETAIL a INNER JOIN TB_ACC b ON a.BPD_ACC=b.ACC_CODE "
                 + "WHERE a.BPD_BPM_MASTER = ?";
            PreparedStatement pres2 = c.cn().prepareStatement(query2);
            pres2.setString(1, id);
            ResultSet res2 = pres2.executeQuery();
            while (res2.next()) {
                o[0] = res2.getString("BPD_ACC");
                o[1] = res2.getString("ACC_NAME");
                o[2] = OneforAllfunc.nf(res2.getDouble("BPD_AMOUNT"));
                o[3] = res2.getString("BPD_DESC");
                dtm.addRow(o);
            }
                    ...
                    if (id.equals("")) {
                        try {
                            if (OneforAllfunc.accountcheck(pane.edaccount.getText()) == false) {
                                ...
                                String query = "INSERT INTO TB_BP_MASTER (BPM_DOC_NO, BPM_DATE_TRANS, "
                                     + "BPM_REF_NO, BPM_DATE_REF,BPM_ACC) VALUES (?,?,?,?,?);";
                                PreparedStatement pres = c.cn().prepareStatement(query);
                                ...
                                for (int i = 0; i < rowcount; i++) {
                                    String queryin = "INSERT INTO TB_BP_DETAIL (BPD_ID, BPD_BPM_MASTER, "
                                         + "BPD_ACC, BPD_AMOUNT, BPD_DESC) VALUES (?,?,?,?,?);";
                        ...
                        try {
                            if (OneforAllfunc.accountcheck(pane.edaccount.getText()) == false) {
                                ...
                                String query = "UPDATE TB_BP_MASTER SET BPM_DOC_NO=?, BPM_DATE_TRANS=?, "
                                     + "BPM_REF_NO=?, BPM_DATE_REF=?,BPM_ACC=? WHERE BPM_DOC_NO=?;";
                                PreparedStatement pres = c.cn().prepareStatement(query);
                                pres.setString(1, pane.eddoc_no.getText());
                                pres.setString(2, OneforAllfunc.datetodb(pane.eddate_trans.getDate()));
                                pres.setString(3, pane.edref_no.getText());
                                ...
                                pres.executeUpdate();
                                c.dc();
                                String querydel = "DELETE FROM TB_BP_DETAIL WHERE BPD_BPM_MASTER=?;";
